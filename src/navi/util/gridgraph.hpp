#ifndef NAVI_GRIDGRAPH_HPP
#define NAVI_GRIDGRAPH_HPP
...
class gridgraph {
	public:
		gridgraph(const Grid& data) : data(data) {}
...
void put(my_pred_map& m, gridvertex k, gridvertex v) {
	m[k] = v;
}
namespace boost {
	template <>
...
class my_dist_map : public std::map<vertex_descriptor, cost> {
	...
	private:
	typedef std::map<vertex_descriptor, cost> self;
};
