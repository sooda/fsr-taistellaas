...
#include <j2b2wrapper/j2b2wrapper.hpp>
...
#else
		cout << "Starting epic j2b2 wrapper:" << endl;
		J2B2Wrapper::start();
		cout << "Shuffling while retrieving the sensor map:" << endl;
		while (! J2B2Wrapper::sensorMapComputed()){
			usleep(500000);
			cout << "oh yeah " << flush;
		}
		gpt->sensorMap=J2B2Wrapper::sensorMap();
		cout << "Connected, wohoo!" << endl;
...
#define printParam(n)\
	...
	list<RangeReading*> rrlist;
	if (gpt->onLine){
		cout << " oh yeah we are online baby, starting the good stuff " << endl;
		RangeReading rr(0,0);
		while (1){
			// we are going to stay here for awhile ...
			while (J2B2Wrapper::getReading(rr)){
				RangeReading* nr=new RangeReading(rr);
				rrlist.push_back(nr);
				gpt->processScan(*nr);
			}
