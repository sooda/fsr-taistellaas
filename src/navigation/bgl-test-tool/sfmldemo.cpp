...
int main()
{
	...
	GridMap grid(gh, GridRow(gw));
	GridMap dilagrid(gh, GridRow(gw));
	VectorGrid container(grid);

	int lastx = 0;
	int lasty = 0;
	const int DILATION_AMOUNT = 3;
	while (App.IsOpened())
	{
		// Process events
		sf::Event Event;
		...
		if (pressed) {
			grid[mgy][mgx] = true;
			dilagrid = dilate(grid, DILATION_AMOUNT);
		}
		...
		if (!(lastx == mgx && lasty == mgy) && !grid[mgy][mgx] && inp.IsKeyDown(sf::Key::Space)) {
			boost::tie(l, p) = gridsearch(dilagrid, gridvertex(mgx, mgy, &container));
		...
		for (int y = 0; y < gh; y++) {
			for (int x = 0; x < gw; x++) {
				if (dilagrid[y][x]) {
					box(App, x, y, sf::Color(128, 128, 128));
				}
			}
		}
		for (int y = 0; y < gh; y++) {
