...
CSLAMDemo::CSLAMDemo(CJ2B2Client &aInterface)
...
int CSLAMDemo::RunInfoDemo(int aIterations)
{
  int iterations = 0;
  int posSeq = -1;
...
int CSLAMDemo::RunSDLDemo(int aIterations)
{ 
  if (iSDLThreadActive) {
  ...
  while(iDemoActive && 
    ...
    if (iLastLaserDistanceArray.size()) {
      ...
      for(int i = 0; i < dasize; ++i) {
        ...
        int dscale = 1;
	while (int cscale = 1; cscale < scale; cscale += dscale) {
...
int CSLAMDemo::RunCameraDemo(int aIterations)
{
  bool r;
...
int CSLAMDemo::RunMotionDemo(int aIterations)
{
  const float angspeed = M_PI / 2.0;
...
int CSLAMDemo::RunSensorsDemo(int aIterations)
{
  using namespace MaCI;
...
int CSLAMDemo::ThreadFunction(const int aThreadNumber)
{
  int result = 0;
